...
class BimanualAgent(Agent):
    ...
    def get_action(self) -> np.ndarray:
        left_act = self.agent_left.get_action()
        right_act = self.agent_right.get_action()
        left_act = np.array(left_act)
        right_act = np.array(right_act)
        act = np.concatenate([left_act, right_act])
        return act
