...
def _quat_from_rpy(roll, pitch, yaw) -> np.ndarray:
    """Convert an euler angle (Rz(yaw)Rp(pitch)Rx(roll)) representation to a quaternion.

    Args:
        rpy (np.ndarray): The euler angle [roll, pitch, yaw].

    Returns:
        np.ndarray: The quaternion representation of the axis-angle.
    """
    Q = Quaternion(matrix=_rot_from_rpy(roll, pitch, yaw))
    quat = np.array([Q.x, Q.y, Q.z, Q.w])
    return quat


def _rot_from_rpy(roll, pitch, yaw) -> np.ndarray:
    return _rot_yaw(yaw) @ _rot_pitch(pitch) @ _rot_roll(roll)


def _rot_roll(r: float) -> np.ndarray:
    return np.array(
        [
            [1, 0, 0], 
...
def _rot_pitch(p: float) -> np.ndarray:
    return np.array(
        [
...
def _rot_yaw(y: float) -> np.ndarray:
    return np.array(
        [
...
class RobotState:
    ...
    def rot(self) -> np.ndarray:
        return _rot_from_rpy(self.roll, self.pitch, self.yaw)
...
class XArmRobot:
    GRIPPER_OPEN = 800
    GRIPPER_CLOSE = 0
    DEFAULT_MAX_DELTA = 0.001

    def __init__(self, ip: str, control_frequency: float = 50.0, max_delta: float = DEFAULT_MAX_DELTA):
        self.robot = XArmAPI(ip, is_radian=True)
        if self.robot is None:
            raise ValueError("Failed to initialize XArm")
    ...
    def move_and_wait_robot(self, joint_pos: np.ndarray):
        if np.any(np.abs(joint_pos) > np.pi):
            raise ValueError("abs of joint pos should not larger than pi")
        self.command_joint_state(joint_pos)
        print("Waiting for the robot to move to the target position")
        while True:
            cur_joint_state = self.get_joint_state()[:-1]
            if np.allclose(cur_joint_state, joint_pos, atol=1e-4):
                time.sleep(2.0)
                break
            else:
                time.sleep(0.1)
    ...
    def _robot_thread(self):
        ...
        while self.running:
            ...
            if gripper_command is not None:
                ...
                self._set_gripper_position(self.GRIPPER_OPEN + set_point * (self.GRIPPER_CLOSE - self.GRIPPER_OPEN))
            self.last_state = self._update_last_state()
            ...
            if count % 1000 == 0:
                ...
                # print(f"Step time - mean: {np.mean(step_times):10.3f}, std: {np.std(step_times):10.3f},
                # min: {np.min(step_times):10.3f}, max: {np.max(step_times):10.3f}")
                print(
                    f"Low  Level Frequency - mean: {frequency:10.3f}, \
                    std: {np.std(frequency):10.3f}, \
                    min: {np.min(frequency):10.3f}, max: {np.max(frequency):10.3f}"
                )
                step_times = []
    ...
    def _get_gripper_pos(self) -> float:
        ...
        normalized_gripper_pos = (gripper_pos - self.GRIPPER_OPEN) / (self.GRIPPER_CLOSE - self.GRIPPER_OPEN)
...
def main():
    robot = XArmRobot(ip="192.168.1.220")
