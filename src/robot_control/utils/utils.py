...
def get_root(path: Union[str, Path], name: str = ".root") -> Path:
    """
    Walk upward from the given path until a file named `name` (default: '.root') is found.
    Returns the directory containing it.
    
    Raises:
        FileNotFoundError: if no such file exists in any parent directory.
    """
    ...
    raise FileNotFoundError(
        f"Could not find '{name}' in '{path}' or any of its parent directories."
    )
